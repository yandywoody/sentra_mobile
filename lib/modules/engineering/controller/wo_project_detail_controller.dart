...
class WoProjectDetailController extends GetxController {
  WoProjectModel data = Get.arguments;
  ...
  WoProjectRepository repo = WoProjectRepository();

  WoProjectEmployeeRepository woEmpRepo = WoProjectEmployeeRepository();
  ...
  WoProjectRealizationRepository woRealizRepo =
  ...
  checkWoOnprogres() async {
    WoOnProgressRepository woOnProgressRepo = WoOnProgressRepository();
    var _dtOnProgress =
        await woOnProgressRepo.getDataByWo(data.code, "Project");
  ...
  getWoOnprogres() async {
    WoOnProgressRepository woOnProgressRepo = WoOnProgressRepository();
    var _dtOnProgress = await woOnProgressRepo.getDataByWo(data.id, "Project");
    dtOnProgress.value = _dtOnProgress;
    if (dtOnProgress.length > 0) {
  ...
  startTimer() {
    if (!isGoodReceived.value) {
      ...
      Get.defaultDialog(
        title: "Info",
        middleText: "Siap memulai Project?",
        textCancel: "Batal",
        textConfirm: "Mulai",
        onConfirm: () async {
          Get.back();
  ...
  pauseWo() async {
    Get.defaultDialog(
        ...
        onConfirm: () async {
          ...
          WoOnProgressRepository woOnProgressRepo = WoOnProgressRepository();
          var _dtOnProgress =
              await woOnProgressRepo.getDataByWo(data.id, "Project");
  ...
  addRealization(var woProgress) {
    ...
    WoProjectRealizationModel realiztionMdl = WoProjectRealizationModel(
      id: realizId,
      projectDetailId: data.id,
      code: data.code,
      activityCode: data.name,
      machineCode: "-",
      employeeId: loginuser.employeeId,
    ...
    dtWoAll = WoAllRealizationModel(
      id: realiztionMdl.id,
      relationableId: realiztionMdl.projectDetailId,
  ...
  stopTimer(WoProjectModel dt, String type) {
    statusWo.value = type;
  ...
  getWoProgress() async {
    ...
    if (dtOnProgress[0].status == 2) {
      ...
      timeDuration.value = (dtOnProgress[0].durationPause ??
              0 + int.parse(diff.inMinutes.toString()))
          .toString();
    }
    timer?.cancel();
  ...
  getWoEmp() async {
    var dt = woEmpRepo.getEmpByWo(data.code);
